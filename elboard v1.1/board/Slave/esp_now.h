...
#define ESP_NOW_MAX_DATA_LEN         16000       /*!< Maximum length of ESPNOW data which is sent very time */
...
/**
  ...
  */
typedef void (*esp_now_recv_cb_t)(const uint8_t *mac_addr, const uint16_t *data, int data_len);
...
/**
  ...
  */
esp_err_t esp_now_send(const uint8_t *peer_addr, const uint16_t *data, size_t len);
